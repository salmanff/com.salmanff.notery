...
var notery;
const NOTERY_META_INIT = {
  // console.log(' this needs to be cleaned up')
  appName: 'com.salmanff.notery'
}
...
var startup = function() {
	...
	if (freezrMeta.userId && freezrMeta.userId != notery.data.freezrMeta.userId){
		if (confirm("There is data from another user on your device. If you press okay, that data will be deleted.")) {
			...
			notery.data.freezrMeta.userId = freezrMeta.userId;
...
var removeJlos = function() {
	...
	notery.data.freezrMeta = NOTERY_META_INIT;
...
var createConflictedNote = function(copyOfReturnedConflictedItem, itemInList) {
	...
	if (confirm("There was a conflict syncing your note on "+copyOfReturnedConflictedItem.title+". Do you want to create a new copy of the note?") ) {
		...
		copyOfReturnedConflictedItem.body = "<p>COPY OF POST MADE WITH CHANGES DOWNLOADED on "+freezr.utils.longDateFormat(new Date().getTime())+"</p> "+copyOfReturnedConflictedItem.body;
		let newNote = notery.add("posts",copyOfReturnedConflictedItem);
		resultIndex = notery.idIndex("posts", newNote, true);
...
var addLeftPanelElementInDateOrder = function(listItemNumber, section) {
	...
	if (section=="local") {
		var noteToInsert = notery.data.posts[listItemNumber];
		...
		var noteToInsert = onlineSearch.posts[listItemNumber] ;
		while (nextPostNum && nextPostNum<onlineSearch.posts.length && getMaxLastModDate(onlineSearch.posts[nextPostNum]) > getMaxLastModDate(noteToInsert)) {
			nextPost = nextPost.nextSibling;
			nextPostNum = (nextPost && nextPost.id)? nextPost.id.split('_')[3]: null;
		}
	}
...
var xtraMenuToggle = function(doClose) {
	...
	if (!doClose) {
		...
		document.getElementById("xtras_openFreezrToLogin").style.display =(!freezr.app.isWebBased && !notery.data.freezrMeta.userId)? "block":"none";
		...
		document.getElementById("xtras_encyptOuter").style.display = notery.data.freezrMeta.userId?"block":"none";
...
var doSyncPosts = function () {
	...
	if (!freezr.app.isWebBased && !freezrMeta.adminUser) {
		...
		console.warn("error: cant sync with bad password");
		...
		notery.sync("posts", {
			...
			handleConflictedItem:createConflictedNote,
...
var syncGotNewPosts = function(newPosts, changedPosts) {
	...
	changedPosts.forEach(function (aPost ){
		...
		if (aPost.fj_deleted) {
			...
			addLeftPanelElementInDateOrder(resultIndex,"local");
			if (resultIndex == curr_post_pointer.num && curr_post_pointer.section == "local") {
				showCurrentNote();
			}
		}
	});
...
var syncWarningCB = function(msgJson) {
	if (!msgJson) msgJson = {msg:"Internal unknown error"}
...
const showSyncEndMessages = function (aMsg){
	if (aMsg && (aMsg.status==401 || aMsg.code == "authentication")) {
		console.warn(aMsg)
		if (freezr.app.isWebBased) {
			window.open("/account/login",null);
		} else {
...
var doSearchOnline = function(what) {
	...
	var queryOptions = {
		...
		q:{'_date_modified':{'$lt':onlineSearch.last_retrieved}}
	}

	if (wordsToFind.length==1) {
		queryOptions.q.headers = wordsToFind[0];
		...
		queryOptions.q.$and = wordsToFindQueryList;
	}

	showWarning();
	if (!freezrMeta.userId) {
		showWarning("Please log in to your freezr to fetch your online notes. (Click upper right logo)");
	} else {
		freezr.feps.postquery (queryOptions,
			function(error, returnJson) {
				returnJson = freezr.utils.parse(returnJson);
				if (error) {
          showWarning("There was an error getting data ",error)
          console.warn(error)
        } else if (returnJson && returnJson.length>0) {
					onlineSearch.noMoreLeft = (returnJson.length < onlineSearch.COUNT);
					returnJson.forEach(function (aResult){
						if (notery.idIndex("posts",aResult, false)<0) {
							onlineSearch.posts.push(JSON.parse(JSON.stringify(aResult)));
							if (aResult._date_modified < onlineSearch.last_retrieved) onlineSearch.last_retrieved = aResult._date_modified
	...
	if (!isMobile() && (notery.data.local_backups_do || forceSave)) {
		if (forceSave || !notery.data.local_backups_last_save || (notery.data.local_backups_changed_since_last_save && (now - notery.data.local_backups_last_save > notery.data.local_backup_interval) ) ){
			...
			var text = JSON.stringify(jsontext);
 			var filename = "notery backup for "+freezrMeta.userId+" "+(new Date().getTime())+".json";
...
var nowSavePassword = function(thePassword, firstTimeSetting) {
	if (!notery.data.freezrMeta.userId) return false;
	if (!firstTimeSetting) {
...
var encryptedPost = function(aPost) {
	...
	if (notery.data.encryptDo && stats.encryptPW) {
		...
		// review oct 2017
		//onsole.log("NOT ENCYPTING? "+notery.data.encryptDo)
